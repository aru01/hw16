...
class BooksController extends Controller
{
    ...
    public function retrieve(Book $book) {

      Gate::authorize("retrieve", $book);
      
      $book->status = 0;
      $book->save();
     
      $users = BookReservation::with("user")
      ->where("status", "=", 0)
      ->where("book_id", "=", $book->id)
      ->get()
      ->map(fn($r) = $r->user);

      Mail::to($users)->send(new BookIsInactive($book));

      // return view("dummy");
      return redirect()->route("book", ["book" => $book]);
    }  
}
